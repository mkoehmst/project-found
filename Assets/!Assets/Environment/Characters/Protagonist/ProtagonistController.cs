namespace ProjectFound.Environment.Characters
{
	...
	public class ProtagonistController : MonoBehaviour
	{
		...
		void Update( )
		{
			...
			{
				//agent.enabled = true;
				m_navMeshSpeed = agent.velocity.magnitude;
		...
		void LateUpdate( )
		{
			...
			else
			{
				...
				if ( Misc.Floater.GreaterThan( m_navMeshSpeed, 0f ) )
				{
					//private void CalculateLocalMovement(ref Vector3 worldSpaceMovement)
					//{
					Vector3 worldSpaceMovement = agent.velocity;

					if ( worldSpaceMovement.sqrMagnitude > 1f )
					{
						worldSpaceMovement.Normalize( );
					}

					Vector3 localSpaceMovement = transform.InverseTransformDirection( worldSpaceMovement );
					float turnAmount = Mathf.Atan2( localSpaceMovement.x, localSpaceMovement.z );
					//float forwardAmount = localSpaceMovement.z;
					//}

					//private void ApplyRootRotation()
					//{
					float turnSpeed = 1100f;
...
Autelia.Coroutines.CoroutineController.StartCoroutine(_Jump());
				}
			}
			else{
				canDoubleJump = true;
		...
		void AirControl( )
		{
			if ( !isGrounded )
			{
				...
				{
					velocityX = GetComponentInChildren<Rigidbody>( ).velocity.x - maxVelocity;
					if ( velocityX < 0 )
					{
						velocityX = 0;
					}
				...
				if ( weapon == Weapon.UNARMED )
				{
					...
					if ( attackSide != 3 )
					{
						...
						if ( leftWeapon == 12 || leftWeapon == 14 || rightWeapon == 13 || rightWeapon == 15 )
						{
							...
							Autelia.Coroutines.CoroutineController.StartCoroutine( _LockMovementAndAttack( 0, .75f ) );
						}
						else
						{

							Autelia.Coroutines.CoroutineController.StartCoroutine( _LockMovementAndAttack( 0, .6f ) );
						}
					}
					else
					{
		...
		public void AttackKick( int kickSide )
		{
			if ( isGrounded )
			{
				...
				Autelia.Coroutines.CoroutineController.StartCoroutine( _LockMovementAndAttack( 0, .8f ) );
			}
		}
		...
		public void GetHit( )
		{
			...
			Autelia.Coroutines.CoroutineController.StartCoroutine( _LockMovementAndAttack( .1f, .4f ) );
			//Apply directional knockback force.
			if ( hitNumber <= 2 )
			{

				Autelia.Coroutines.CoroutineController.StartCoroutine( _Knockback( -transform.forward, 8, 4 ) );
			}
			else if ( hitNumber == 3 )
			{

				Autelia.Coroutines.CoroutineController.StartCoroutine( _Knockback( -transform.right, 8, 4 ) );
			}
			else if ( hitNumber == 4 )
			{
		...
		{
			isKnockback = true;
			Autelia.Coroutines.CoroutineController.StartCoroutine( _KnockbackForce( knockDirection, knockBackAmount, variableAmount ) );
...
Autelia.Coroutines.CoroutineController.StartCoroutine(_DirectionalRoll());
				}
			}*/
		}
		...
		public IEnumerator _DirectionalRoll( )
		{
			...
			if ( angle360 > 315 || angle360 < 45 )
			{
				...
				Autelia.Coroutines.CoroutineController.StartCoroutine( _Roll( 1 ) );
			}
			if ( angle360 > 45 && angle360 < 135 )
			{

				Autelia.Coroutines.CoroutineController.StartCoroutine( _Roll( 2 ) );
			}
			if ( angle360 > 135 && angle360 < 225 )
			{

				Autelia.Coroutines.CoroutineController.StartCoroutine( _Roll( 3 ) );
			}
			if ( angle360 > 225 && angle360 < 315 )
			{

				Autelia.Coroutines.CoroutineController.StartCoroutine( _Roll( 4 ) );
			}
			yield return null;
		}
		...
		public void Death( )
		{
			...
			Autelia.Coroutines.CoroutineController.StartCoroutine( _LockMovementAndAttack( .1f, 1.5f ) );
			isDead = true;
			animator.SetBool( "Moving", false );
